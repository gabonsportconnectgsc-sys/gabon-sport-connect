...
`;
  ...
  function injectModal() {
    ...
    div.innerHTML = `
      <div class="gsc-pe-modal">
        ...
        <div class="gsc-pe-field" id="pe-nationalite-wrap"><label>Nationalité</label><input id="pe-nationalite" placeholder="Gabonaise, Camerounaise, Sénégalaise…"></div>
        <div id="pe-org-fields" style="display:none;">
          <div class="gsc-pe-field"><label>Sigle</label><input id="pe-org-sigle" placeholder="ex: FC105"></div>
          <div class="gsc-pe-field"><label>Slogan</label><input id="pe-org-slogan" placeholder="Slogan de la structure"></div>
          <div class="gsc-pe-field"><label>Date de création</label><input id="pe-org-datecreation" type="date"></div>
          <div class="gsc-pe-field"><label>Président / responsable</label><input id="pe-org-president" placeholder="Nom complet"></div>
          <div class="gsc-pe-field"><label>Téléphone du président</label><input id="pe-org-president-tel" placeholder="+241..."></div>
          <div class="gsc-pe-field"><label>Effectif (joueurs/athlètes)</label><input id="pe-org-effectif" type="number" min="0"></div>
          <div class="gsc-pe-field"><label>Catégories encadrées</label><input id="pe-org-categories" placeholder="ex: Seniors, U20, U17…"></div>
          <div class="gsc-pe-field"><label>Siège social / adresse</label><input id="pe-org-siege" placeholder="Adresse"></div>
          <div class="gsc-pe-field"><label>Géolocalisation (lat, lng)</label><input id="pe-org-geoloc" placeholder="ex: 0.39241, 9.45280"></div>
          <div class="gsc-pe-field"><label>WhatsApp officiel</label><input id="pe-org-whatsapp" placeholder="+241..."></div>
          <div class="gsc-pe-field"><label>Infrastructures</label><input id="pe-org-infra" placeholder="Stade, siège, internat…"></div>
          <div class="gsc-pe-field"><label>Biographie / présentation</label><input id="pe-org-bio" placeholder="Présentation de la structure"></div>
          <div class="gsc-pe-field"><label>Palmarès</label><input id="pe-org-palmares" placeholder="Distinctions, titres…"></div>
        </div>
  ...
  function toggleClaimVisibility() {
    ...
    const orgWrap = document.getElementById('pe-org-fields');
    if (orgWrap) orgWrap.style.display = ORG_ROLES.includes(role) ? 'block' : 'none';
    const natWrap = document.getElementById('pe-nationalite-wrap');
    if (natWrap) natWrap.style.display = ORG_ROLES.includes(role) ? 'none' : 'block';
  }
  ...
  function openProfileEditor(uid) {
    ...
    document.getElementById('pe-nationalite').value = u.nationalite || '';
    document.getElementById('pe-claimed').checked = !!u.claimed;
    document.getElementById('pe-org-sigle').value = u.sigle || '';
    document.getElementById('pe-org-slogan').value = u.slogan || '';
    document.getElementById('pe-org-datecreation').value = u.dateCreation || '';
    document.getElementById('pe-org-president').value = (u.gouvernance && u.gouvernance.president) || '';
    document.getElementById('pe-org-president-tel').value = (u.gouvernance && u.gouvernance.presidentTelephone) || '';
    document.getElementById('pe-org-effectif').value = u.effectifTotal || '';
    document.getElementById('pe-org-categories').value = u.categoriesEncadrees || '';
    document.getElementById('pe-org-siege').value = u.siegeSocial || '';
    document.getElementById('pe-org-geoloc').value = (u.geoloc && u.geoloc.lat != null) ? `${u.geoloc.lat}, ${u.geoloc.lng}` : '';
    document.getElementById('pe-org-whatsapp').value = u.whatsapp || '';
    document.getElementById('pe-org-infra').value = u.infrastructures || '';
    document.getElementById('pe-org-bio').value = u.bio || '';
    document.getElementById('pe-org-palmares').value = u.palmares || '';
  ...
  async function saveProfile() {
    ...
    const fields = {
      ...
      nationalite: document.getElementById('pe-nationalite').value.trim(),
    ...
    if (ORG_ROLES.includes(role)) {
      const geolocRaw = document.getElementById('pe-org-geoloc').value.trim();
      let geoloc = null;
      if (geolocRaw) {
        const parts = geolocRaw.split(',').map(p => Number(p.trim()));
        if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) geoloc = { lat: parts[0], lng: parts[1] };
      }
      Object.assign(fields, {
        sigle: document.getElementById('pe-org-sigle').value.trim(),
        slogan: document.getElementById('pe-org-slogan').value.trim(),
        dateCreation: document.getElementById('pe-org-datecreation').value,
        gouvernance: {
          president: document.getElementById('pe-org-president').value.trim(),
          presidentTelephone: document.getElementById('pe-org-president-tel').value.trim()
        },
        effectifTotal: document.getElementById('pe-org-effectif').value,
        categoriesEncadrees: document.getElementById('pe-org-categories').value.trim(),
        siegeSocial: document.getElementById('pe-org-siege').value.trim(),
        geoloc,
        whatsapp: document.getElementById('pe-org-whatsapp').value.trim(),
        infrastructures: document.getElementById('pe-org-infra').value.trim(),
        bio: document.getElementById('pe-org-bio').value.trim(),
        palmares: document.getElementById('pe-org-palmares').value.trim()
      });
    }
