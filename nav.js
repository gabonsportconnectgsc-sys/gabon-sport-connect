...
/* Retire des favoris locaux tout identifiant d'annonce qui n'existe plus
   parmi les annonces actuellement disponibles (annonce supprimée ou passée
   en "occupé"). Corrige le badge ❤️ qui restait bloqué sur un chiffre
   (ex. "1") : sans carte correspondante à l'écran, il n'y avait plus de
   bouton pour retirer ce favori-là. À appeler avec la liste des ids
   d'annonces actuellement chargées, dès qu'elle est connue. */
export function purgerFavorisInexistants(idsDisponibles) {
  const favoris = getFavoris();
  const propres = favoris.filter(id => idsDisponibles.includes(id));
  if (propres.length !== favoris.length) {
    localStorage.setItem(CLE_FAVORIS, JSON.stringify(propres));
    majBadgeFavoris();
  }
}
